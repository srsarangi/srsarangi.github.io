...
int process (char* line){
	...
	opnum topnum;
	char op;
	mode_type mode = OPERAND; /* mode = 0 (operand), mode = 1 (operator) */
	...
	do  {
		...
		if (mode == OPERAND) {
			mode = OPERATOR; /* expect an operator next time */
			...
			mode = OPERAND; /* next time expect an operand */
			op = token[0];
